...
叙事语言规范（同次写作内自检）：
以项目小说语体和 characterPerformances 为准，叙述贴近玩家当下可感知的事物。以下只约束语言表达，不扩大事实授权，也不替代原请求、过程演出和时间结算要求。
- 旁白不要凭空设置误解再用“不是……而是……”“看似……实则……”翻案；直接写有依据的观察。实际误认后的修正、人物否认和有意反讽应保留，不能为了避句式删掉否定、转折或不确定性。
- 相邻句若反复使用相同句法、同样补充和收尾，仅调整必要的句内语序；三项以上的顿号罗列可变换句法，但保留每项必要信息。按顺序发生的行动、问答、角色口癖和有意排比不据此改写。
- 避免旁白用“说白了”“先说结论”、提示性冒号或无必要的破折号给普通观察制造揭晓感。正常引语、对话停顿、被打断的话和有作用的文学表达保留，不按标点数量判错。
- 不用理想化职业人格比喻替代已写明的行为；不把普通比喻、拟人、问句、长句或被动句一概当作问题，不强行口语化、变换句长或拆分演出行。
...
若 authorizedActionOutcomes 包含 cycle-boundary: 来源，程序已确定本次到达当日行动终止边界，玩家选择不能延后它。正文必须据此修改行动收尾：已完成部分与后果→准确的触发原因→当天行动中断，并在最后一段旁白之前添加“效果|loop-transition”。不能只说累了后继续调查，不能把资源耗尽改成午夜，不能完成边界之后的计划。此处只演出确定发生的原因与中断；随后进入已解锁结局还是08:00重置由程序按结局优先规则决定，当前正文不得宣告已经重置或进入结局；不得擅自切换次日地点或修改状态。审查必须指出遗漏或矛盾并要求编剧修改，不能因为存在效果指令就认定内容符合；正文、摘要、选项不得暗示当天仍可继续执行。
...
ActionAuditRequirements 为 null 时顶层 $.actionAudit 返回 null，不要求辅助清单或历史无执行包补演行动；否则按上述唯一 schema 完整返回 $.actionAudit，与 $.continuityAudit、$.assertionAudit 同级。原问题、实际后续及每个程序 segmentId 都须审查。事件中断且没有可执行工作时仍完整返回对象，将程序标记不适用的条目填 not-applicable 和 evidenceLineIndices: []，不能省略整块审查。
...
actionAudit 不复制 quote，只在 evidenceLineIndices 中按升序选择 CharacterContinuityEvidence.lines 的零起点 lineIndex；可选择不相邻的多行，不重复、不臆造编号。程序会从本次可见正文回填逐字引文，不把控制指令、选项或摘要当作正文。适用项 pass 必须选择至少一行并说明这些行如何回应原问题或呈现具体阶段，行号存在不等于语义充分；fail 若整项缺失可选择 [] 并说明遗漏，否则选择违规行；not-applicable 使用 []。任一 fail 都必须拒绝并给出最小修复要求，顶层 approved 不能覆盖失败。
...
例外：EMPTY_PLAYABLE_SCENE、DEATH_NEWS_NOT_DELIVERED、PREMATURE_MIDNIGHT、CYCLE_RESET_NOT_RENDERED 属于演出契约，演出契约纠错优先于保留原文含义。空正文须把获准情节写成 maintext 中实际可播放台词；死讯缺失须只补足已授权的事件及接收反应，不得新增死因或凶手；提前午夜须改成符合权威时钟的当下；重置收尾缺失须依据 cycle-boundary: 权威来源修改已执行行动的后果与中断，最后一段旁白前加“效果|loop-transition”，不得继续当天行动或补写未执行结果。后续进入结局还是08:00重置由程序决定，当前正文不得提前宣告。同步纠正受影响摘要和选项，仍须通过后续事实审核。
例外：UNRESOLVED_OPTION_ACTION 只允许对报错索引的选项做最小改写，使用 approvedSteps 或 optionIntents 已支持的精确注册地点与公开称呼；不得发明目的地、目标、人物或改换行动意图。无法确定时仅保留原获准计划支持的明确行动。

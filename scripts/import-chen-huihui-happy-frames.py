"""Clean and install Chen Huihui's supplied happy talk/blink sheet."""
...
SOURCE = Path(
    r"C:\Users\a\AppData\Local\Temp\codex-clipboard-776120a4-34c1-405c-b8a3-3182504e081e.png"
)
OUTPUT = ROOT / "public/assets/characters/animated/chen-huihui/talk-happy-cleaned"
STATIC_OUTPUT = ROOT / "public/assets/characters/chen-huihui-happy.png"
GRID = (4, 4)
FRAME_SIZE = (720, 1264)
CANVAS_SIZE = (430, 606)
EXPECTED_FRAMES = 14
...
def remove_trapped_background(frame: Image.Image) -> tuple[Image.Image, int]:
    ...
    for y in range(height):
        for x in range(width):
            ...
            raised_arm_background = (
                min_x == 0
                and min_y >= height * 0.45
                and max_y == height - 1
                and len(component) >= 10_000
            )
            if hair_gap or left_hair_cavity or right_hair_cavity or raised_arm_background:
...
def is_bottom_matte(pixel: tuple[int, int, int, int]) -> bool:
    red, green, blue, alpha = pixel
    return alpha > 0 and min(red, green, blue) >= 60 and max(red, green, blue) - min(red, green, blue) <= 45
...
def remove_bottom_white_residue(frame: Image.Image) -> tuple[Image.Image, int]:
    ...
    seeds = [(x, height - 1) for x in range(min_x, max_x + 1) if is_bottom_matte(pixels[x, height - 1])]
    if not seeds:
...
def normalize_to_canvas(frame: Image.Image) -> Image.Image:
    ...
    canvas.alpha_composite(resized, ((canvas_width - target_size[0]) // 2, canvas_height - target_size[1]))
    return canvas
...
def main() -> None:
    ...
    for index in range(GRID[0] * GRID[1]):
        column = index % GRID[0]
        row = index // GRID[0]
        frame = sheet.crop((column * 720, row * 1264, (column + 1) * 720, (row + 1) * 1264))
        ...
        print(f"{destination.name}: removed {removed_background} trapped pixels; {removed_bottom} bottom pixels")

"""Clean and install Zhou Deming's calm/default animation frames.

The supplied sheet is a 4x2 RGBA grid. Its only unwanted matte is a narrow,
near-white wedge between the trouser legs that touches the bottom edge of each
frame. Restricting the flood fill to that lower-centre region prevents pale
skin, shirt and candy details from being altered.
"""
...
SOURCE = Path(
    r"C:\Users\a\AppData\Local\Temp\codex-clipboard-41452a52-9089-47da-bcab-a14c543299b4.png"
)
OUTPUT = ROOT / "public/assets/characters/animated/old-man/talk-calm-cleaned"
GRID = (4, 2)
FRAME_SIZE = (720, 1280)
CANVAS_SIZE = (430, 606)
...
def remove_bottom_white_wedge(frame: Image.Image) -> tuple[Image.Image, int]:
    ...
    # The confirmed residue occupies x=354..374 and y=1210..1279 on the
    # 720x1280 source frames. Keep a small safety margin while staying well
    # inside the trouser-leg region.
...
def normalize_to_canvas(frame: Image.Image) -> Image.Image:
    ...
    # Premultiplied resizing avoids a pale halo from RGB values hidden under

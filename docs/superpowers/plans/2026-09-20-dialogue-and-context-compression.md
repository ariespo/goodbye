...
4. Project prompt history without editing the original messages. Default threshold is 60,000 estimated tokens (raised from 12,000 at the user's request), player-adjustable from 2,000 to 100,000. Compress older assistant narratives first, retain the latest two narrative turns in full where the model's hard budget permits. Missing summaries retain original text; summaries must not increase history size. Route-version isolation, valid commitments and facts keep existing authority. Existing saved thresholds stay unchanged; clearing the setting and saving restores the current default.
